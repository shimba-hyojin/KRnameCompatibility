/* ===================================================================
   GitHub Pages 정적판용 API — 서버 없이 브라우저에서 전부 처리
   -------------------------------------------------------------------
   * 획수표와 궁합 알고리즘은 backend/hangul.py · compatibility.py 와 동일.
     따라서 EC2 버전과 항상 같은 점수가 나온다.
   * 랭킹은 localStorage 에 저장 → 새로고침·재방문해도 남는다.
     단 "그 브라우저에만" 남는다 (사람마다 각자의 랭킹).
   * 포춘쿠키는 로컬 메시지 목록에서 뽑는다 (외부 API는 CORS로 막히므로).
   =================================================================== */
(function () {
  'use strict';

  var STORAGE_KEY = 'namecompat.ranking.v1';
  ...
  // ---------------------------------------------------------- storage
  // localStorage 는 시크릿 모드나 저장 용량 초과 시 예외를 던진다.
  // 실패하면 메모리에만 담고 계속 동작한다.
  var memoryStore = {};
  ...
  function ranking(limit) {
    ...
    var rows = Object.keys(store).map(function (name) {
      return {
        ...
        created_at: store[name].created_at || null
      };
    }).sort(function (x, y) {
      return y.score - x.score || x.partner_name.localeCompare(y.partner_name);
    }).slice(0, limit || 20);
  ...
  window.fetch = function (url, options) {
    ...
    if (method === 'DELETE') {
      ...
      if (m) {
        ...
        if (!store[target]) {
          return respond(
            { deleted: 0, message: '「' + target + '」の記録は見つかりませんでした。' }, 404
          );
        }
        delete store[target];
        saveStore(store);
    ...
    if (path.indexOf('/compatibility') === 0) {
      ...
      if (!name) {
        return respond({ error: 'EMPTY', message: 'お名前を入力してください。' }, 400);
      }
      if (!name.split('').every(isSyllable)) {
        return respond(
          { error: 'NOT_HANGUL', message: 'ハングル（한글）で入力してください。例: 야마다' }, 400
        );
      }
